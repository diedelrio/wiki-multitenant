...
export default function Header({ onMenu, project, user, onLogout }) {
  ...
  return (
    <><header className="topbar">
      ...
      <div className="topbar-actions"><Link className="icon-button" to="/" aria-label="Cambiar proyecto" title="Cambiar proyecto"><FolderKanban size={18}/></Link>{user.globalRole === 'SUPERADMIN' && <Link className="icon-button" to="/admin" aria-label="Administración global" title="Administración global"><Shield size={18}/></Link>}<button className="icon-button" onClick={() => setAboutOpen(true)} aria-label="Acerca de la wiki" title="Acerca de"><Info size={18}/></button><ThemeToggle /><button className="icon-button" onClick={onLogout} aria-label="Cerrar sesión" title="Cerrar sesión"><LogOut size={18} /></button></div>
